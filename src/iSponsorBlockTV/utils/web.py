...
class SetupServer:
    ...
    async def get_config(self, request: web.Request) -> web.Response:
        return web.json_response(
            {
                "devices": [
                    {"name": d.name, "screen_id": d.screen_id}
    ...
    async def pair_device(self, request: web.Request) -> web.Response:
        try:
            ...
            pairing_code = (
                str(data["pairing_code"]).strip().replace("-", "").replace(" ", "")
            )
    ...
    async def delete_device(self, request: web.Request) -> web.Response:
        try:
            ...
            if not screen_id:
                return web.json_response(
                    {"error": "Missing screen_id parameter"}, status=400
                )

            # Filter out the device with the given screen_id
            original_length = len(self.config.devices)
            self.config.devices = [
                d for d in self.config.devices if d.screen_id != screen_id
            ]
...
def run_web_server(data_dir, port=8080, headless=False):
    """Run the web server as a standalone process"""
...
def main(data_dir: str):
    """Main entry point for the web configuration UI"""
    run_web_server(data_dir, port=8080, headless=False)
